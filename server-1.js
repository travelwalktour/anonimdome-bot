...
// ── /add — работает и для СБП и для Крипто ──
bot.onText(/\/add(\d+)/, async (msg, match) => {
  if (String(msg.chat.id) !== ADMIN_CHAT_ID) return;
  const id = match[1];
  // Ищем в обоих списках
  const p = pending_sbp[id] || pending_crypto[id];
  ...
  delete pending_sbp[id];
  delete pending_crypto[id];
...
// ── /deny — работает и для СБП и для Крипто ──
bot.onText(/\/deny(\d+)/, async (msg, match) => {
  ...
  bot.sendMessage(msg.chat.id, `❌ Заявка ${id} отклонена`);
});

// ── Поддержка — пересылка сообщений ──
bot.on('message', async (msg) => {
  // Игнорируем команды и сообщения от админа
  if (String(msg.chat.id) === ADMIN_CHAT_ID) return;
  if (msg.text && msg.text.startsWith('/')) return;
  
  const userId = msg.chat.id;
  const username = msg.chat.username ? '@' + msg.chat.username : 'без username';
  const text = msg.text || '[не текст]';
  
  try {
    await bot.sendMessage(ADMIN_CHAT_ID,
      `💬 Сообщение от пользователя\n👤 ID: ${userId} (${username})\n\n${text}\n\nОтветить: /reply${userId} текст`
    );
  } catch(e){}
});
...
bot.onText(/\/reply(\d+) (.+)/, async (msg, match) => {
  ...
  const text = match[2];
  try {
    await bot.sendMessage(userId, `💬 Поддержка:\n\n${text}`);
    bot.sendMessage(msg.chat.id, `✅ Сообщение отправлено пользователю ${userId}`);
  } catch(e) {
    bot.sendMessage(msg.chat.id, `❌ Не удалось отправить. Пользователь: ${userId}`);
  }
});

// ── Поддержка через форму ──
...
bot.on('callback_query', async (query) => {
  ...
  pendingReplies[String(query.message.chat.id)] = userId;
  await bot.answerCallbackQuery(query.id, { text: 'Напишите ответ!' });
  await bot.sendMessage(query.message.chat.id,
    `✏️ Напишите ответ пользователю ${userId}:`,
    { reply_markup: JSON.stringify({ force_reply: true, selective: false }) }
  );
});

// ── Отправка ответа клиенту (любое сообщение от админа = ответ) ──
bot.on('message', async (msg) => {
  if (String(msg.chat.id) !== ADMIN_CHAT_ID) return;
  if (!msg.text || msg.text.startsWith('/')) return;
  
  const userId = pendingReplies[String(msg.chat.id)];
  if (!userId) return;
  
  try {
    await bot.sendMessage(userId, `💬 Ответ поддержки:\n\n${msg.text}`);
    await bot.sendMessage(msg.chat.id, `✅ Ответ отправлен!`);
    delete pendingReplies[String(msg.chat.id)];
  } catch(e) {
    await bot.sendMessage(msg.chat.id, `❌ Не удалось отправить пользователю ${userId}`);
  }
});
