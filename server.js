...
bot.on('callback_query', async (query) => {
  ...
  pendingReplies[query.message.chat.id] = userId;
...
bot.on('message', async (msg) => {
  ...
  const userId = pendingReplies[msg.chat.id];
  if (!userId) return;
  
  try {
    await bot.sendMessage(userId, `💬 Ответ поддержки:\n\n${msg.text}`);
    await bot.sendMessage(msg.chat.id, `✅ Ответ отправлен пользователю ${userId}`);
    delete pendingReplies[msg.chat.id];
  } catch(e) {
    await bot.sendMessage(msg.chat.id, `❌ Не удалось отправить`);
  }
});
